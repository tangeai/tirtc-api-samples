...
extension _DemoPlayerMediaFileActions on _DemoPlayerPageState {
  Future<void> _toggleRecording() async {
    ...
    if (_session.isRecording) {
      ...
      if (result.success) {
        widget.smokeMarkerSink?.passed('smoke_recording_stopped', payload: <String, Object?>{
          'file_path': result.data!.path,
          'duration_ms': result.data!.duration.inMilliseconds,
        });
      } else {
        _smokeFail(
          failureStage: 'media_recording_stop',
      ...
      if (result.success) {
        ...
        _smokeFail(
          failureStage: 'media_recording_start',
          message: 'recording start failed',
          errorCode: result.code,
        );
      }
      _showPlayerSnack(
        result.success ? '已开始本地保存' : '开始保存失败 · ${TiRtc.formatError(result.code ?? 0)}',
      );
    }
  ...
  Future<void> _takeSnapshot() async {
    ...
    if (result.success) {
      widget.smokeMarkerSink?.passed('smoke_snapshot_saved', payload: <String, Object?>{
        'file_path': result.data!.path,
      });
    } else {
      _smokeFail(
        failureStage: 'media_snapshot',
        message: 'snapshot failed',
        errorCode: result.code,
      );
    }
    _showPlayerSnack(
      result.success ? '截图已保存 · ${result.data?.path}' : '截图失败 · ${TiRtc.formatError(result.code ?? 0)}',
    );
    _setMediaFileBusy(false);
  }
  ...
  Future<void> _moveLatestMediaToGallery() async {
    ...
    final Resp<TiRtcGalleryAsset> result = await _session.moveLatestMediaToGallery();
    if (result.success) {
      final String marker = mediaType == 'video' ? 'smoke_recording_gallery_saved' : 'smoke_snapshot_gallery_saved';
      widget.smokeMarkerSink?.passed(marker, payload: <String, Object?>{
      ...
      _smokeFail(
        failureStage: 'media_gallery',
        message: 'media gallery move failed',
        errorCode: result.code,
      );
    }
    _showPlayerSnack(
      result.success ? '已保存到系统相册' : '保存失败 · ${TiRtc.formatError(result.code ?? 0)}',
    );
    _setMediaFileBusy(false);
  }
}
