...
class PlayerCommandButton extends StatelessWidget {
  const PlayerCommandButton({super.key, required this.onOpenCommands});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: OutlinedButton(
        ...
        child: const Text('发送命令', style: TextStyle(fontSize: 12), textAlign: TextAlign.center),
      ),
    );
  }
}
...
class DownlinkControlButton extends StatelessWidget {
  const DownlinkControlButton({super.key, required this.connecting, required this.playing, required this.onPressed});
  ...
  Widget build(BuildContext context) {
    return FilledButton.icon(
      ...
      label: Text(connecting ? '连接中' : (playing ? '停止播放' : '开始播放')),
    );
  }
}
